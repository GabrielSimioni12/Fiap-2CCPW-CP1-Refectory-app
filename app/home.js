...
export default function Home() {
  // Puxamos a lista de favoritos do contexto
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {search.length === 0 && (
          <>
            ...
            {/* NOVA SECÇÃO: MEUS FAVORITOS (Só aparece se houver favoritos) */}
            {favorites.length > 0 && (
              <View style={styles.section}>
                <View style={styles.favoriteHeader}>
                  <Ionicons name="heart" size={20} color="#ED145B" />
                  <Text style={styles.categoryTitleFav}>Meus Favoritos</Text>
                </View>
                <FlatList
                  data={favorites}
...
const styles = StyleSheet.create({
  ...
  // Estilos da secção de Favoritos
  favoriteHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: 15 },
  categoryTitleFav: { fontSize: 22, fontWeight: '800', color: '#333', marginLeft: 8 },
